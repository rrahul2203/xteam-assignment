...
from src.qa.embeddings import DocumentVectors, can_encode, load_model
...
class TestFallback:
    ...
    def test_hybrid_degrades_to_lexical_without_overstating_what_ran(
            self, kb, block_sentence_transformers):
        """Hybrid falls back and says so, and still resolves dates.

        Date resolution is asserted here because the date layer is independent of ranking: losing
        the optional dependency must cost semantic scoring only, not the correct version.
        """
        retriever = Retriever(kb, mode=MODE_HYBRID)
        assert retriever.describe() == MODE_TFIDF

        hits = retriever.search(DISPUTE, AS_OF, k=3)
        assert hits and hits[0].lexical > 0
...
class TestFusion:
    ...
    def test_fused_score_is_the_weighted_sum_of_the_parts(self, kb):
        ...
        Scores are raw cosines, not min-max normalised, which is what keeps the absolute
        abstention threshold meaningful -- so the arithmetic is pinned rather than assumed. The
        weight is then driven to each extreme, where a hybrid has to reproduce the pure mode's
        ranking exactly: that is what makes the weight a dial between two signals rather than a
        third scoring rule.
        """
        weight = 0.45
        hits = Retriever(kb, mode=MODE_HYBRID, embedding_weight=weight).search(DISPUTE, AS_OF, k=5)
        top = hits[0]
...
class TestModelLoading:
    def test_the_model_is_built_once_from_the_local_checkout(self, monkeypatch, tmp_path):
        """Two guarantees at once: loading from disk keeps the first question off the network,
        and caching keeps the model from being rebuilt per question.
        """
        builds = []
...
class TestDocumentVectors:
    def test_the_cached_fixture_covers_the_whole_kb_and_is_normalised(self, kb):
        ...
        matrix, _ = vectors.matrix(list(kb.values()))
